...
"SELECT * FROM customer_card WHERE percent=%PAR%;",
"SELECT selling_price, products_number, product_name, characteristics FROM store_product INNER JOIN product p on store_product.id_product = p.id_product WHERE UPC=%PAR%;",

// MANAGER QUERIES
"SELECT * FROM `check` WHERE id_employee=%PAR1% AND (print_date BETWEEN %PAR2% AND %PAR3%);",
"SELECT * FROM `check` WHERE check_number=%PAR%;",
"SELECT * FROM customer_card WHERE cust_surname=%PAR%;",
"SELECT * FROM customer_card WHERE percent=%PAR%;",
"SELECT * FROM product WHERE category_number=%PAR% ORDER BY product_name;",
"SELECT * FROM product ORDER BY product_name;",
...
"SELECT store_product.UPC, UPC_prom, id_product, store_product.selling_price, store_product.products_number, promotional_product FROM store_product INNER JOIN sale s on store_product.UPC = s.UPC WHERE check_number=%PAR%;",
"SELECT selling_price, products_number FROM store_product INNER JOIN product p on store_product.id_product = p.id_product WHERE UPC=%PAR%;",
"SELECT id_employee, empl_surname, empl_name, empl_patronymic, role, salary, date_of_birth, date_of_start, phone_number, city, street, zip_code FROM employee WHERE id_employee=%PAR%;",
];
